...
export const Route = createFileRoute('/staff/medicalequipmentpage_screening')({
  component: MedicalEquipmentPageScreening,
});
...
const sidebarDepts = [
  ...
  { label: "รังสีวินิจฉัย", sublabel: "Diagnostic Radiology", icon: "🩺", route: "/staff/medicalequipmentpage_sceening", active: true },
];
...
function ActionLink({ label, color }: { label: string; color: string }) {
  ...
  return (
    <button
      ...
      style={{ background: h ? color + "15" : "transparent", border: "none", color, fontSize: 11, fontWeight: 600, cursor: "pointer", padding: "3px 6px", borderRadius: 6, textAlign: "left", textDecoration: h ? "underline" : "none", transition: "all 0.15s" }}
    >
      🔗 {label}
    </button>
  );
...
function EquipmentCard({ eq }: { eq: Equipment }) {
  ...
  return (
    <div
      ...
      style={{ background: hovered ? "#fff5f8" : "#ffffff", border: hovered ? "1.5px solid #e879a0" : "1.5px solid #e5e7eb", borderRadius: 16, padding: "12px 12px 10px", display: "flex", flexDirection: "column", alignItems: "center", gap: 8, cursor: "pointer", transition: "all 0.2s ease", boxShadow: hovered ? "0 4px 20px rgba(232,121,160,0.18)" : "0 1px 4px rgba(0,0,0,0.06)", minWidth: 140, maxWidth: 170, flex: "0 0 auto" }}
    >
      ...
      <div style={{ background: "#1e40af", color: "#fff", fontSize: 11, fontWeight: 700, padding: "3px 10px", borderRadius: 6, textAlign: "center", width: "100%" }}>{eq.label}</div>
...
function Sidebar() {
  ...
  return (
    <aside style={{ width: 260, flexShrink: 0, background: "#ffffff", borderRight: "1px solid #f3e8f0", display: "flex", flexDirection: "column", padding: "20px 0", minHeight: "100vh", boxShadow: "2px 0 12px rgba(232,121,160,0.07)" }}>
      ...
      <div style={{ padding: "0 20px 12px", fontSize: 10, fontWeight: 700, color: "#9ca3af", letterSpacing: 1.2, textTransform: "uppercase" }}>งานการพยาบาล</div>
      {sidebarDepts.map((dept) => (
        <button
          key={dept.label}
          onClick={() => navigate({ to: dept.route as any })}
          style={{ background: dept.active ? "linear-gradient(90deg, #fce7f3, #fdf2f8)" : "transparent", border: "none", borderLeft: dept.active ? "3px solid #e879a0" : "3px solid transparent", padding: "10px 20px", cursor: "pointer", display: "flex", alignItems: "center", gap: 10, textAlign: "left", transition: "all 0.15s", marginBottom: 2, width: "100%" }}
        >
          <span style={{ fontSize: 18 }}>{dept.icon}</span>
          <div>
            <div style={{ fontSize: 13, fontWeight: dept.active ? 700 : 500, color: dept.active ? "#be185d" : "#374151", fontFamily: "'Noto Sans Thai', 'Sarabun', sans-serif", lineHeight: 1.3 }}>{dept.label}</div>
            {dept.sublabel && <div style={{ fontSize: 10, color: dept.active ? "#e879a0" : "#9ca3af", fontWeight: 500 }}>{dept.sublabel}</div>}
          </div>
        </button>
      ))}
    </aside>
...
// ---- Main Component ----
function MedicalEquipmentPageScreening() {
      ...
      <main style={{ flex: 1, padding: "28px 32px", overflowY: "auto" }}>
        <div style={{ marginBottom: 24 }}>
          <div style={{ fontSize: 12, color: "#9ca3af", marginBottom: 8, display: "flex", alignItems: "center", gap: 6 }}>
            <span>งานการพยาบาลรังสีวิทยา</span><span>›</span><span>รังสีวินิจฉัย</span><span>›</span>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            <div>
              ...
              <p style={{ margin: "4px 0 0", fontSize: 13, color: "#6b7280" }}>ฝ่ายรังสีวินิจฉัย (Diagnostic Radiology) · โรงพยาบาลธรรมศาสตร์เฉลิมพระเกียรติ</p>
            </div>
